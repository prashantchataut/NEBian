...
export default function AnswerError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4 px-4 animate-scale-in">
      ...
      <h2 className="text-lg font-semibold text-on-surface">Failed to load answer page</h2>
      <p className="text-sm text-on-surface-variant text-center max-w-sm">
        Something went wrong while loading the answer page. Please try again.
      </p>
      <button
        onClick={reset}
